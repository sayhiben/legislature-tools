import {
  parsePixelLikeValue as parsePixelLikeValueShared,
  toBool as toBoolShared,
  toEpochMillis as toEpochMillisShared,
  toFiniteNumberOrNull as toFiniteNumberOrNullShared,
  toNumber as toNumberShared,
} from "./shared/coercion.js";
import {
  formatDurationHumanized as formatDurationHumanizedShared,
  formatPercent as formatPercentShared,
  formatRatio as formatRatioShared,
} from "./shared/formatting.js";
import { ReportStateStore } from "./state/report_state_store.js";
import { ChartDataRepository } from "./data/chart_data_repository.js";
import { buildDefaultChartRendererRegistry } from "./charts/default_renderer_registry.js";
import { filterRowsByDuplicateCollisionControls as filterRowsByDuplicateCollisionControlsModule } from "./controls/duplicate_collision_filters.js";
import {
  TABLE_SEMANTIC_CLASS_NAMES,
  semanticClassForTableCell as semanticClassForTableCellModule,
} from "./tables/semantic_rules.js";
import {
  parseBucketFromLocalStorage as parseBucketFromLocalStorageModule,
  parseBucketFromQueryParams as parseBucketFromQueryParamsModule,
  parseLinkedZoomFromQueryParams as parseLinkedZoomFromQueryParamsModule,
  parseOptionFromQueryParams as parseOptionFromQueryParamsModule,
  syncControlOverridesToUrl as syncControlOverridesToUrlModule,
  updateUrlQueryParams as updateUrlQueryParamsModule,
} from "./adapters/url_state.js";

export async function runReportApp() {
  ...
  const toEpochMillis = toEpochMillisShared;
  ...
  const parsePixelLikeValue = parsePixelLikeValueShared;
  ...
  const formatRatio = formatRatioShared;
  ...
  const formatDurationHumanized = formatDurationHumanizedShared;
  ...
  function updateUrlQueryParams(mutator) {
    updateUrlQueryParamsModule(mutator);
  }

  function syncControlOverridesToUrl() {
    syncControlOverridesToUrlModule(state);
  }
  ...
  function setActiveTocHeading(headingId, syncUrl) {
    ...
    state.setTocHeading(normalized);
  ...
  function normalizeBucketKey(value) {
    return chartDataRepository.normalizeBucketKey(value);
  }

  function chartRowsFromMap(payload) {
    return chartDataRepository.chartRowsFromMap(payload);
  }
  ...
  const chartDataRepository = new ChartDataRepository({
    chartShardManifestByAnalysis: chartShardManifestByAnalysis,
    chartBaseRowsMap: chartBaseRowsMap,
    chartBucketRowsMap: chartBucketRowsMap,
    chartBucketOptionsByChart: chartBucketOptionsByChart,
    toFiniteNumberOrNull: toFiniteNumberOrNull,
    fetchJsonPayload: fetchJsonPayload,
  });

  function mergeChartShardPayload(payload, bucketKey) {
    chartDataRepository.mergeChartShardPayload(payload, bucketKey);
  }

  function getChartBucketOptions(chartId, rows) {
    return chartDataRepository.getChartBucketOptions(chartId, rows);
  }

  function resolveBucketTarget(options) {
    return chartDataRepository.resolveBucketTarget(options, state.activeBucket, state.defaultBucket);
  }

  function chartShardEntryForAnalysis(analysisId) {
    return chartDataRepository.chartShardEntryForAnalysis(analysisId);
  }

  async function ensureAnalysisBaseLoaded(analysisId) {
    await chartDataRepository.ensureAnalysisBaseLoaded(analysisId);
  }

  function analysisBucketSet(analysisId) {
    return chartDataRepository.analysisBucketSet(analysisId);
  }

  function analysisBucketPendingMap(analysisId) {
    return chartDataRepository.analysisBucketPendingMap(analysisId);
  }

  async function ensureAnalysisBucketLoaded(analysisId, bucketMinutes) {
    await chartDataRepository.ensureAnalysisBucketLoaded(analysisId, bucketMinutes);
  }

  function resolvedBucketsForAnalysis(analysisId) {
    return chartDataRepository.resolvedBucketsForAnalysis(
      analysisId,
      state.activeBucket,
      state.defaultBucket
    );
  ...
  function analysisNeedsShardLoad(analysisId) {
    return chartDataRepository.analysisNeedsShardLoad(
      analysisId,
      state.activeBucket,
      state.defaultBucket
    );
  ...
  async function ensureAnalysisDataLoaded(analysisId) {
    await chartDataRepository.ensureAnalysisDataLoaded(
      analysisId,
      state.activeBucket,
      state.defaultBucket
    );
  ...
  function getChartRows(chartId) {
    return chartDataRepository.getChartRows(chartId, state.activeBucket, state.defaultBucket);
  }

  function uniqueBucketOptions(rows) {
    return chartDataRepository.uniqueBucketOptions(rows);
  }
  ...
  function parseBucketFromQueryParams(availableOptions) {
    return parseBucketFromQueryParamsModule(availableOptions, toFiniteNumberOrNull);
  }

  function parseBucketFromLocalStorage(availableOptions) {
    return parseBucketFromLocalStorageModule(
  ...
  function parseLinkedZoomFromQueryParams() {
    return parseLinkedZoomFromQueryParamsModule();
  }

  function parseDuplicateOptionFromQueryParams(names, options) {
    return parseOptionFromQueryParamsModule(names, options);
  }
  ...
  function propagateZoom(minTime, maxTime, sourceChartId, isReset) {
    ...
    try {
      if (isReset) {
        ...
        state.setZoomRange(minTime, maxTime);
      }
  ...
  const chartRendererRegistry = buildDefaultChartRendererRegistry({
    getState: () => state,
    normalizeReportMatchMode: normalizeReportMatchMode,
    renderDateHourHeatmap: renderDateHourHeatmap,
    ...
    renderTimeBarLine: renderTimeBarLine,
  });
  ...
  function renderAutoChart(mount, rows) {
    ...
    const registryResult = chartRendererRegistry.render(mount.chartId, mount, rows, {
      state: state,
    });
  ...
  const tableSemanticClassNames = TABLE_SEMANTIC_CLASS_NAMES;
  ...
  function semanticClassForTableCell(tableKey, field, value) {
    return semanticClassForTableCellModule(tableKey, field, value);
  }
  ...
  function filterRowsByDuplicateCollisionControls(chartId, rows) {
    return filterRowsByDuplicateCollisionControlsModule({
      chartId: chartId,
      rows: rows,
      state: state,
      normalizeReportMatchMode: normalizeReportMatchMode,
  ...
  function initDuplicateCollisionControls() {
    ...
    let initialDuplicateScope = queryScope
      ? queryScope
      : scopeOptions.includes(savedScope)
        ? savedScope
        : state.defaultDuplicateScope;
    let initialDuplicateMetric = queryMetric
      ? queryMetric
      : metricOptions.includes(savedMetric)
        ? savedMetric
        : state.defaultDuplicateMetric;
    let initialDuplicateMatchMode = queryMatchMode
    ...
    if (!scopeOptions.includes(initialDuplicateScope)) {
      initialDuplicateScope = scopeOptions[0];
    }
    if (!metricOptions.includes(initialDuplicateMetric)) {
      initialDuplicateMetric = metricOptions[0];
    }
    if (!matchModeOptions.includes(initialDuplicateMatchMode)) {
      initialDuplicateMatchMode = matchModeOptions[0];
    }
    state.setDuplicateFilters({
      scope: initialDuplicateScope,
      metric: initialDuplicateMetric,
      matchMode: initialDuplicateMatchMode,
    });
  ...
  function initVoterMatchModeControls() {
    ...
    let initialVoterMatchMode = queryMode
    ...
    if (!modeOptions.includes(initialVoterMatchMode)) {
      initialVoterMatchMode = modeOptions[0];
    }
    state.setVoterMatchMode(initialVoterMatchMode);
    ...
    const onChange = () => {
      state.setVoterMatchMode(
        normalizeReportMatchMode(matchModeSelect.value, state.defaultVoterMatchMode || "loose")
      );
      try {
